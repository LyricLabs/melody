...
transaction(id: UInt64) {
  ...
  execute {
     self.receiverCollection.deposit(token: <- Melody.claimTicket(userCertificateCap: self.userCertificateCap, vault: <- self.vault, reciever: receiver, config: self.config))
  }
}
